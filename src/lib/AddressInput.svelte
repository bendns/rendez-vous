...
<div class="relative flex flex-col gap-2">
  <div
    class="flex items-center gap-3 bg-white rounded-2xl shadow-sm border border-gray-100 px-4 py-3 transition-all duration-200 focus-within:shadow-md focus-within:border-coral/30"
  >
  ...
  <div class="relative">
    ...
    >
      ...
      <input
        ...
        placeholder={friends.list.length >= MAX_ADDRESSES
          ? t('friends.maxReached', { max: MAX_ADDRESSES })
          : t('friends.addressPlaceholder')}
        disabled={friends.list.length >= MAX_ADDRESSES}
        class="flex-1 bg-transparent outline-none text-dark placeholder:text-light text-[15px] disabled:opacity-50 disabled:cursor-not-allowed"
      />
      {#if loading}
    ...
    {#if showSuggestions && suggestions.length > 0}
      <ul
        class="absolute z-50 w-full mt-2 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden animate-slide-down"
      >
