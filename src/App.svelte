...
<div class="h-screen flex flex-col overflow-hidden bg-warm-white">
  <!-- Header -->
  <header
    class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-white/80 backdrop-blur-md border-b border-gray-100 z-10"
  >
    ...
    <div class="flex items-center gap-1.5 sm:gap-2">
      ...
      <div class="flex bg-warm-gray rounded-lg p-0.5 gap-0.5">
        {#each locales as lang (lang)}
          <button
            ...
            class="px-1.5 sm:px-2 py-1 rounded-md text-xs font-semibold transition-all duration-200 cursor-pointer border-none {locale.value ===
            lang
              ? 'bg-white shadow-sm text-coral'
              : 'bg-transparent text-medium hover:text-dark'}">{localeLabels[lang]}</button
          >
        {/each}
      </div>
      <!-- Unit selector -->
      <div class="flex bg-warm-gray rounded-lg p-0.5 gap-0.5">
        <button
          ...
          class="px-2 sm:px-2.5 py-1 rounded-md text-xs font-semibold transition-all duration-200 cursor-pointer border-none {unit.value ===
          'km'
            ? 'bg-white shadow-sm text-coral'
            : 'bg-transparent text-medium hover:text-dark'}">km</button
        >
        <button
          ...
          'mi'
            ? 'bg-white shadow-sm text-coral'
            : 'bg-transparent text-medium hover:text-dark'}">mi</button
        >
      </div>
      <button
        onclick={() => (showSettings = !showSettings)}
        class="px-2 sm:px-2.5 py-1 rounded-lg text-sm cursor-pointer border-none transition-colors {showSettings
          ? 'bg-coral/10 text-coral'
          : 'bg-warm-gray text-medium hover:text-dark'}"
        title={t('settings')}
      >
        {apiKey.value ? '🔑' : '⚙️'}
      </button>
  ...
  {#if showSettings}
    <div
      class="px-4 sm:px-6 py-3 bg-warm-gray/50 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 animate-fade-in"
    >
      ...
      <div class="flex items-center gap-2 flex-1">
        ...
        {:else}
          <a
            ...
            target="_blank"
            rel="noopener"
            class="text-xs text-coral hover:underline whitespace-nowrap shrink-0">{t('settings.getKey')}</a
          >
        {/if}
      </div>
    </div>
  ...
  <div class="flex-1 flex overflow-hidden">
    <!-- Sidebar -->
    <aside
      class="w-full md:w-[380px] lg:w-[420px] shrink-0 flex flex-col bg-warm-white border-r border-gray-100 overflow-hidden {sidebarOpen
        ? ''
        ...
        {#if friends.list.length >= 2 && !tooFarApart.value}
          ...
          <div class="animate-fade-in">
            ...
            <div class="flex bg-warm-gray rounded-xl p-1 gap-1">
              <button
                onclick={() => {
                  ranking.value = 'equidistant';
                  rerankVenues();
                }}
                class="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer border-none {ranking.value ===
                'equidistant'
                  ? 'bg-white shadow-sm text-coral'
              ...
              <button
                onclick={() => {
                  ...
                  ranking.value = 'walking';
                  rerankVenues();
                }}
                class="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all duration-200 border-none {!apiKey.value
                  ? 'bg-transparent text-light cursor-default'
                  : ranking.value === 'walking'
                    ? 'bg-white shadow-sm text-coral cursor-pointer'
                    : 'bg-transparent text-medium hover:text-dark cursor-pointer'}"
              >
                <span class="text-lg">🚶</span>
                {t('ranking.walkTime')}
                {#if !apiKey.value}
    ...
    <main class="flex-1 relative {sidebarOpen ? 'hidden md:block' : ''}">
      <MapView bind:selectedVenue {sidebarOpen} />

      <!-- Floating info -->
      {#if friends.list.length >= 2 && venues.list.length > 0}
        <div
          class="absolute top-3 left-3 sm:top-4 sm:left-4 bg-white/90 backdrop-blur-md rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 shadow-lg border border-gray-100 animate-fade-in"
        >
          <p class="text-xs text-medium">
            <span class="font-bold text-coral">{venues.list.length}</span>
            {mode.value === 'restaurant' ? t('info.restaurants') : t('info.bars')}
            {t('info.found')}
            · {t('info.rankedBy')}
            {ranking.value === 'walking' ? t('info.walkTime') : t('info.distance')}
          </p>
        </div>
      {/if}
    </main>
